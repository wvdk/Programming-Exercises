...
class Environment {
    ...
    func lookup(symbol: Symbol) -> Expression? {
        return vars[symbol.name] is Expression ? vars[symbol.name] : outer?.lookup(symbol)
    }
}
...
class Evaluator {
    ...
    func getNextChar(input: String, nextIndex: String.Index) -> String {
        return input.substringWithRange(nextIndex..<advance(nextIndex, 1))
    }
    ...
    func nvs /* number values */ (es: [Expression], env: Environment) -> [Int] {
        return map(es[1..<countElements(es)]){ self.nv(self.evalr($0, env: env)) }
    }
    
    func narithmetic(ns: [Int], op: (Int, Int) -> Int) -> Number {
        return Number(value: reduce(ns[1..<countElements(ns)], ns[0], op))
    }
